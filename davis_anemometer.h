...
#define DAVISSTRING 120 // Must include space for terminal null character
char DAVISstring[DAVISSTRING]; // Output buffer for DAVIS string

void updateDAVISstring() {
  //"Davis Anemometer":{"init":true,"direction":53,"avg speed":0,"min speed":0,"max speed":0,"rotations":0}

  jsonStart(DAVISstring, "Davis Anemometer");

  jsonInit(DAVISstring, "init");
  if (anemometerData.status == true) {
    // Output true
    strcat(DAVISstring, "true");
  } else {
    // Output false
    strcat(DAVISstring, "false");
  }
  jsonComma(DAVISstring);

  jsonInit(DAVISstring, "direction");
  sprintf(dummyString, "%d", anemometerData.direction); // Yay we can use sprintf for int type
  strcat(DAVISstring, dummyString);
  jsonComma(DAVISstring);

  jsonInit(DAVISstring, "avg speed");
  dtostrf(anemometerData.avgSpeed, 4, 6, dummyString);  //4 is mininum width, 2 is precision
  strcat(DAVISstring, dummyString);
  jsonComma(DAVISstring);

  jsonInit(DAVISstring, "min speed");
  dtostrf(anemometerData.minSpeed, 4, 6, dummyString);  //4 is mininum width, 2 is precision
  strcat(DAVISstring, dummyString);
  jsonComma(DAVISstring);

  jsonInit(DAVISstring, "max speed");
  dtostrf(anemometerData.maxSpeed, 4, 6, dummyString);  //4 is mininum width, 2 is precision
  strcat(DAVISstring, dummyString);
  jsonComma(DAVISstring);

  jsonInit(DAVISstring, "rotations");
  sprintf(dummyString, "%d", anemometerData.rotations); // Yay we can use sprintf as it is an int
  strcat(DAVISstring, dummyString);
